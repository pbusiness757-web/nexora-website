...
const NODES: Node[] = [
  { key: 'USDT', label: 'USDT', x: 20, y: 18, variant: 'crypto', delay: '0ms' },
  { key: 'BTC', label: 'BTC', x: 50, y: 12, variant: 'crypto', delay: '200ms' },
  { key: 'ETH', label: 'ETH', x: 80, y: 18, variant: 'crypto', delay: '400ms' },
  { key: 'NEXORA', label: 'Nexora', x: 50, y: 50, variant: 'hub', delay: '0ms' },
  { key: 'corporate', x: 24, y: 82, variant: 'output', delay: '300ms' },
  { key: 'bankCard', x: 50, y: 88, variant: 'output', delay: '500ms' },
  { key: 'localCurrency', x: 76, y: 82, variant: 'output', delay: '700ms' },
];
...
export default function HeroSection() {
  ...
  const nodeLabel = (node: Node) =>
    node.variant === 'output'
      ? dict.hero.network.nodes[
          node.key as keyof typeof dict.hero.network.nodes
        ]
      : (node.label ?? node.key);
  ...
  return (
    <section className="relative overflow-hidden bg-slate-50">
      <div className="mx-auto grid max-w-7xl grid-cols-1 items-center gap-12 px-6 py-16 lg:grid-cols-2 lg:py-20">
        ...
        >
          <div
            className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-2xl shadow-slate-200 transition-transform duration-300 ease-out [transform-style:preserve-3d] motion-reduce:!transform-none"
            style={{
          ...
          >
            <div className="mb-6 flex items-center justify-between">
              <span className="text-sm font-semibold text-slate-500">
                {dict.hero.network.title}
              </span>
              <span className="rounded-full bg-emerald-50 px-3 py-1 text-xs font-semibold text-emerald-600">
                {dict.hero.network.live}
              </span>
            </div>

            <div className="relative aspect-square w-full [transform-style:preserve-3d]">
              <svg
              ...
              {NODES.map((node) => (
                ...
                >
                  <span
                    className="nexora-float relative flex items-center justify-center"
                    style={{ animationDelay: node.delay }}
                  >
                    <span
                    ...
                    />
                    <span
                      className={`relative whitespace-nowrap rounded-full px-3 py-1.5 text-xs font-semibold shadow-sm ${nodeStyles[node.variant]}`}
                    >
                      {nodeLabel(node)}
                    </span>
                  </span>
      ...
      <div className="mx-auto max-w-7xl px-6 pb-16">
        <SmartImage
          ...
          className="h-48 w-full rounded-[2rem] border border-slate-200 shadow-lg shadow-slate-200/60 md:h-72"
        />
      </div>
    </section>
  );
