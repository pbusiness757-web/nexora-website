...
import { clientApi, RequestDetail } from "@/lib/clientApi";
...
export default function RequestDetailPage() {
  ...
  if (loading) {
    return <div className="text-gray-400 text-sm text-center py-12">Загрузка...</div>;
  }
  ...
  if (!req) {
    return (
      <div className="text-center py-12">
        ...
        <Link href="/cabinet/requests" className="text-indigo-400 hover:text-indigo-300 text-sm">
          ← Назад к заявкам
        </Link>
      </div>
    );
  ...
  return (
    <div>
            ...
            {STATUS_ORDER.map((s, i) => {
              ...
              return (
                <div key={s} className="flex items-center flex-shrink-0">
                  ...
                  {!isLast && (
                    <div
                      className={`h-0.5 w-10 mb-4 mx-1 flex-shrink-0 ${done ? "bg-indigo-500" : "bg-gray-700"}`}
                    />
                  )}
                </div>
              );
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
        ...
        {(req.nexoraFeeAmount || req.partnerFeeAmount) && (
          <div className="bg-gray-900 border border-gray-800 rounded-lg p-4">
            ...
            <div className="space-y-2">
              {req.nexoraFeeAmount && (
                <Row label={`Nexora (${req.nexoraFeePercent}%)`} value={`${Number(req.nexoraFeeAmount).toLocaleString("ru-RU")} ${req.payoutCurrency}`} />
              )}
              {req.partnerFeeAmount && (
                <Row label={`Партнёр (${req.partnerFeePercent}%)`} value={`${Number(req.partnerFeeAmount).toLocaleString("ru-RU")} ${req.payoutCurrency}`} />
              )}
            </div>
          </div>
        )}
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-lg p-4 mb-4">
        ...
        {uploadOk && (
          <p className="text-green-400 text-xs mt-2">Файл успешно загружен</p>
        )}
...
function Row({ label, value, highlight }: { label: string; value: string; highlight?: boolean }) {
  return (
    <div className="flex justify-between items-baseline gap-2">
      ...
      <span className={`text-sm text-right ${highlight ? "text-white font-semibold" : "text-gray-200"}`}>
        {value}
      </span>
    </div>
  );
